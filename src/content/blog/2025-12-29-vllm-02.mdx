---
title:  "Learning PagedAttention with VLLM"
pubDate:   '2025-12-29'
author: 'Retep' 
tag: 'Tech'
language: 'EN'
...
`KVCacheManager` does some optimization by removing blocks that will not be used, and calls `coordinator.allocate_new_blocks` to get new `KVCacheBlock`. 
...
`KVCacheBlock` is a data structure that stores block information (`block_id`, `ref_cnt` and hash). Note that it also stores the previous free block and next free block. This is because 
```py
@dataclass
class KVCacheBlock:
