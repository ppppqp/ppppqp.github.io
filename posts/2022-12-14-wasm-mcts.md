...
## SSR?
Server side rendering是个很头疼的问题。现在也没有解决（可以看到棋盘需要刷新一次页面才能显示）。我猜测原因是在server side rendering的时候，不会加载javascript，所以棋盘不会渲染。然后到了client side rendering，初次加载的时候（还没有加载javascript）和SSR的结果比对相同，就直接渲染了。刷新一次才会显示加载完javascript的结果。

奇怪的是如果我用dynamic import来关闭SSR，整个javascript在所有时间都不会被加载。有知道原因的大佬可以告诉我。
